...
def convert(srtfile, txtfile, format, gap):
    ...
    last_end = 0
    state = SUB_NUMBER
    srtfile_ = open(srtfile, 'r', encoding='utf-8-sig')
    txtfile_ = open(txtfile, 'w')
    txt_list = []
    for line in srtfile_.readlines():
        if state == SUB_NUMBER:
            line = line.strip()
            subnum = int(line)
            state = SUB_TIMES
...
#                txtfile_.write("{0:d} {1:s} {2:s} {3:s}".format(subnum, start, convert_timecode(times[1]), txt))
...
if __name__ == '__main__':
    ...
    parser.add_argument('srtfile', nargs='?', type=str,
                        help="Filename for source SRT file.")
    parser.add_argument('txtfile', nargs='?', type=str,
                        help="Destination file for converted Adobe data.")
    args = parser.parse_args()
    srtfile = './The Ingenious Design of the Aluminum Beverage Can.srt'
    txtfile =  './The Ingenious Design of the Aluminum Beverage Can.txt'
    text = convert(srtfile, txtfile, args.format, args.gap)
    text = text.replace("www.engineerguy.com/preview", 'our website')
    sentence_set, sentence_idx = split_sentence(text)
